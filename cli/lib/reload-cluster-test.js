...
var ReloadCluster = (file, opt) => {
  ...
  opt.respawnIntervalManager = new RespawnIntervalManager({minRespawnInterval: opt.minRespawnInterval});
  ...
  /**
   * removes worker from activeWorkers array  (actaully a map)
   * @param w - worker
   */
  ...
  function fork(wid) {
    ...
    console.log(workers)
    console.dir(activeWorkers,{ depth: 2 })
    ...
    if ( Object.keys(tOpeners).length < nW ) {
      ...
      w.process.on('exit', () => {
        removeItem(workers, w);    // slices it out of array
  ...
  self.run = () => {
    ...
    channel.on('online',(w) => {
      console.log(`worker ${w.id} is online ... state: ${w._rc_isReplaced}`)
  ...
  self.reload = function (cb) {
    ...
    for ( var w in cluster.workers ) {
      if ( Object.keys(tClosers).length < nW ) {
        tClosers[w.id] = w
        delete tOpeners[w.id]
      }
    }
